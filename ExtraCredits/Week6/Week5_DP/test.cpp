...
void Graph::countShortest(int u)
{
topSort();

vector<int> C(size, INT_MAX);
C[u] = 0;
for (int i = 0; i < size; i++)
{
    int currNode = topOrder[i];

    if(C[currNode]!=INT_MAX){
        for (int j = 0; j < (int)Adj[currNode].size(); j++)
        {
            int v = Adj[currNode][j];
            int temp = C[currNode] + Weights[currNode][j];
